...
// Calculate BMR whenever physical stats change
useEffect(() => {
// Using Mifflin-St Jeor Equation for BMR calculation
let calculatedBmr = 0;

if (physicalStats.gender === 'male') {
calculatedBmr = 10 * physicalStats.weight + 6.25 * physicalStats.height - 5 * physicalStats.age + 5;
} else {
calculatedBmr = 10 * physicalStats.weight + 6.25 * physicalStats.height - 5 * physicalStats.age - 161;
}
...
calculatedBmr *= activityMultipliers[physicalStats.activityLevel as keyof typeof activityMultipliers];
...
const handleSaveProfile = async () => {
  ...
  try {
    ...
    // If you have a setUser function in your auth context, you'd call it here
    // setUser(data.user);
...
const handleSaveSettings = async () => {
  ...
  try {
    // Include physical stats in the data being sent to the backend
    const dataToSave = {
      notifications,
      privacy,
      theme,
    ...
    // Call your API to save all settings
      ...
      <div className="mt-8 bg-gray-50 p-4 rounded-md">
        ...
        <div className="mt-2 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
          <div>
            ...
            <p className="mt-1 text-2xl font-semibold text-gray-900">{bmr} calories/day</p>
          ...
          <div>
            ...
            <p className="mt-1 text-2xl font-semibold text-gray-900">
              {measurementUnit === 'metric' 
                ? (physicalStats.weight / ((physicalStats.height / 100) * (physicalStats.height / 100))).toFixed(1)
                : (703 * (physicalStats.weight / (physicalStats.height * physicalStats.height))).toFixed(1)}
            </p>
